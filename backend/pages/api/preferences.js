...
router.get('/', (req, res) => {
    ...
    db.get(query, [userId], (err, row) => {
        ...
        res.status(200).json({ preferences: row ? JSON.parse(row.preferences_data) : {} });
    });
});
...
const testPreferencesAPI = async () => {
    const testUserId = 1;
    const testPreferences = { theme: 'light', language: 'fr', notifications: false };
    ...
    // Test getPreferences
    const response = await fetch(`${config.baseUri}:${config.backendPort}/api/preferences?userId=${testUserId}`);
