...
function updateMicList() {
  navigator.mediaDevices.enumerateDevices().then((devices) => {
    ...
    const currentOptions = Array.from(micSelect.options).map(option => option.value);
    ...
    if (selectedValue && newOptions.includes(selectedValue)) micSelect.value = selectedValue;
  });
}
...
function saveRecordingToIndexedDB(fileName, audioBlob, streamId) {
  ...
  request.onsuccess = event => {
    ...
    if (!db.objectStoreNames.contains("recordings")) {
      console.error("Object store 'recordings' is missing. Try clearing the database and reloading the page.");
      return;
    }
