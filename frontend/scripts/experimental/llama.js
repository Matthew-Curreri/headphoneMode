...
async function TransformerCreate (config) {
  ...
  // Precomputed rotary frequency embedding
    ...
    for (let l = 0; l < cfg.n_layers; l++) {
      ...
      for (let h = 0; h < cfg.n_heads; h++) {
        ...
        // Softmax over attention scores (row-wise)
        {
          const bindGroupSoftmax = device.createBindGroup({
            ...
            entries: [{ binding: 0, resource: { buffer: scoresBuffer } }]
          })
      ...
      // Activation (GeLU) on ff1Buf
      {
        const bindGroupGelu = device.createBindGroup({
          ...
          entries: [{ binding: 0, resource: { buffer: ff1Buf } }]
        })
...
const shaderModuleMatMul = device.createShaderModule({
  code: `
...
const shaderModuleAttnScore = device.createShaderModule({
  code: `
    struct Matrix { size: vec2<f32>; numbers: array<f32>; };
...
const shaderModuleSoftmax = device.createShaderModule({
  code: `
    struct Matrix { size: vec2<f32>; numbers: array<f32>; };
...
const shaderModuleRMSNorm = device.createShaderModule({
  code: `
    struct Matrix { size: vec2<f32>; numbers: array<f32>; };
...
const shaderModuleResidualAdd = device.createShaderModule({
  code: `
    struct Matrix { size: vec2<f32>; numbers: array<f32>; };
...
const shaderModuleGelu = device.createShaderModule({
  code: `
    struct Matrix { size: vec2<f32>; numbers: array<f32>; };
...
const shaderModuleRotary = device.createShaderModule({
  code: `
    struct Matrix { size: vec2<f32>; numbers: array<f32>; };
...
const shaderModuleEmbed = device.createShaderModule({
  code: `
    struct Matrix { size: vec2<f32>; numbers: array<f32>; };
