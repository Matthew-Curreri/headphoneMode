class BPETokenizer {
    ...
    constructor(modelData, options = {}) {
      ...
      const regexPattern =
        `${contraction}| ?${letterSeq}| ?${numberSeq}| ?${otherSeq}|\\s`;
    ...
    decode(tokenIds) {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        bytes.push(this.unicodeToByte[char] !== undefined ? this.unicodeToByte[char] : text.charCodeAt(i));
      }
          ...
          if (/^\s+$/.test(sub)) {
            ...
            for (const ch of sub) {
              ...
              if (tokenId !== undefined) {
                ...
                throw new Error(`Whitespace token '${ch}' not found in vocabulary.`);
              }
            }
            continue;
          }
    ...
    _bpeMerge(tokenStr) {
      ...
      if (tokenStr.length <= 1) {
        return this.encoder[tokenStr] !== undefined ? [this.encoder[tokenStr]] : [];
      }
      ...
      return symbols.map(sym => {
        if (!(sym in this.encoder)) {
          throw new Error(`Token '${sym}' not found in vocabulary during BPE merge.`);
        }
